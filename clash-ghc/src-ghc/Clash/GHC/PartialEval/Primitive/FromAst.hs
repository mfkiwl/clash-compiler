...
instance FromAst (SNat n) where
  fromValue (stripValue -> value) =
    case value of
      VData dc [Right n, Left knN] _env -> do
...
instance (FromAst a) => FromAst (Vec n a) where
  fromValue (stripValue -> value) =
    case value of
      VData _dc args _env
        |  [Right _n, Right _a, Left _co] <- args
        -> pure (unsafeCoerce Nil)

        |  [Right n, Right _a, Right _m, Left _co, Left x, Left y] <- args
  ...
  fromValue (stripValue -> value) =
    case value of
      VData _dc args _env
        ...
        |  [Right _n, Right _a, Right _m, Left _co, Left x, Left y] <- args
        -> pure (LCons x y)
