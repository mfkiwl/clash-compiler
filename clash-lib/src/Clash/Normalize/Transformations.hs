...
setupMultiResultPrim _ctx e@(Prim primInfo) = do
  tcm <- Lens.view tcCache
  prim <- Lens.use (extra . primitives . Lens.at (primName primInfo))

  case prim >>= extractPrim of
    Just (BlackBoxHaskell{multiResult=True}) ->
      changed (setupMultiResultPrim' tcm primInfo)
...
setupMultiResultPrim' tcm primInfo@PrimInfo{primType} =
 ...
 where
  ...
  multiPrimBind = mkApps (MultiPrim primInfo) (multiPrimTypeArgs <> multiPrimTermArgs)
