...
import           Clash.Core.Term
  (mpi_resultTypes,  Alt, Pat (..), Term (..), TickInfo (..), PrimInfo(primName), collectArgs
...
mkNetDecl (id_,tm) = preserveVarEnv $ do
  ...
  if | not (shouldRenderDecl hwTy tm) -> return []
     | (MultiPrim pInfo, args) <- collectArgs tm -> multiDecls pInfo args
          ...
          assn  =
            case exprApp of
              ...
              Noop ->
                ...
                -- mealy IO / inline assertions / multi rest primitives.
                []
              _ ->
...
mkExpr bbEasD declType bndr app =
 ...
 in  withTicks ticks $ \tickDecls -> do
  ...
  case appF of
    ...
    MultiPrim pInfo -> mkPrimitive False bbEasD bndr pInfo args tickDecls
    Var f
      | null tmArgs ->
